...
//compute the prediction
  bool predict_brcond (uint32_t PC)
  {
    Tagepred (PC);
...
//Path history
    ...
    if ((brtype & (F_CTRL|F_COND)) == (F_CTRL|F_COND))
      {
	/*local history  */
...
//END UPDATE  HISTORIES
  ...
  void
    update_brcond (uint32_t PC, bool resolveDir, bool predDir,
    		uint32_t branchTarget, uint32_t op_type)
  {

    bool taken = resolveDir;
    uint32_t target = branchTarget;
...
//sign changes: no way it can have been useful
    ...
    HistoryUpdate (PC, op_type, taken, target, phist,
  ...
  int
    Gpredict (uint32_t PC, long long BHIST, int *length, int8_t ** tab, int NBR)
  {
    PERCSUM = 0;
    for (int i = 0; i < NBR; i++)
      {
  ...
  void
    Gupdate (uint32_t PC, bool taken, long long BHIST, int *length,
  ...
  void TrackOtherInst (uint32_t PC, uint32_t branchTarget)
  {
