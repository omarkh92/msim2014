				...
				&& (MD_OP_FLAGS(rs->op) & F_CTRL))
		{
			if (!brupdate_cond || (MD_OP_FLAGS(rs->op) & F_COND)) {
				if (!brupdate && alg1) {
					...
					contexts[rs->context_id].pred->mypred->update_brcond(rs->PC, rs->next_PC != (rs->PC + sizeof(md_inst_t)), br_taken, rs->next_PC, MD_OP_FLAGS(rs->op));
				}
			}
		}
		...
		if ((MD_OP_FLAGS(op) & F_CTRL)) {
			if (!brupdate_cond || (MD_OP_FLAGS(op) & F_COND)) {
				if (brupdate && alg1) {
					...
					contexts[disp_context_id].pred->mypred->update_brcond(regs->regs_PC, regs->regs_NPC != (regs->regs_PC + sizeof(md_inst_t)), br_taken, regs->regs_NPC, MD_OP_FLAGS(op));
				}
			}
		}

		if ((MD_OP_FLAGS(op) & F_CTRL) || op == RETN) {
			if (MD_OP_FLAGS(op) & F_DIRJMP || MD_OP_FLAGS(op) & F_INDIRJMP || op == RETN || MD_OP_FLAGS(op) & F_UNCOND) {
				contexts[disp_context_id].pred->mypred->TrackOtherInst (regs->regs_PC, regs->regs_NPC);
			}
		}
